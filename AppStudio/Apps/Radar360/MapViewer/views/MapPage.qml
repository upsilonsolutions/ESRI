...
Controls.BasePage {
    id: mapPage
    // Muhammad Ali - Dataset and Feature declaration...
    property var layerFeatureSet
    property var popupDefinitionInfo
    property var oldLat
    property var oldLong
    // END Muhammad Ali - Dataset and Feature declaration...
    ...
    contentItem: Rectangle {
        ...
        MapView {
            ...
            property QtObject identifyProperties: QtObject {
                ...
                {
                    clearHighlight()
                    // Muhammad Ali - Highlight feature...
                    if(featuregeometry==null){
                        zoom = true
                        var point = ArcGISRuntimeEnvironment.createObject("Point", {x: layerFeatureSet[0].geometry.x , y: layerFeatureSet[0].geometry.y});


                        var simpleMarker = ArcGISRuntimeEnvironment.createObject("SimpleMarkerSymbol",
                                                                                 {color: "cyan", size: app.units(10),
                                                                                     style: Enums.SimpleMarkerSymbolStyleCircle}),
                        graphic = ArcGISRuntimeEnvironment.createObject("Graphic", {symbol: simpleMarker, geometry:point   })
                    ...
                    }else if (featuregeometry.geometryType === Enums.GeometryTypePoint) {
                        var simpleMarker = ArcGISRuntimeEnvironment.createObject("SimpleMarkerSymbol",
                                                                                 {color: "cyan", size: app.units(10),
                                                                                     style: Enums.SimpleMarkerSymbolStyleCircle}),
            ...
            locationDisplay {
                positionSource: PositionSource {
                    ...
                    onPositionChanged:{
                        // Muhammad Ali - Location Monitor...
                        // need to change...
                        var dist = getDistanceBetweenTwoPoints(oldLat,
                                                                  oldLong,
                                                                  position.coordinate.latitude,
                                                                 position.coordinate.longitude)
                        oldLat = position.coordinate.latitude
                        oldLong = position.coordinate.longitude
                        console.log(dist)
                        if(dist<1 && mapView.identifyProperties.popupManagers.length>0){
                            return
                        }


                        if(locationAccuracy.accuracy <= accuracyThresholdInMeters) {
                            if(layerFeatureSet){
                                var distanceArray = []


                                for(var i=0;i<layerFeatureSet.length;i++){
                                 var dist = getDistanceBetweenTwoPoints(layerFeatureSet[i].geometry.y,
                                                                           layerFeatureSet[i].geometry.x,
                                                                           position.coordinate.latitude,
                                                                          position.coordinate.longitude)
                                    if(dist<distanceThresholdInMeters){
                                        distanceArray.push({"index":i,
                                                               "distance":dist})
                                    }
                                }
                                if(distanceArray.length>0){
                                    distanceArray.sort(function compareNumbers( a, b ) { return a.distance - b.distance} )
                                    var closestitem =  layerFeatureSet[distanceArray[0].index]
                                    if(mapView.identifyProperties.popupManagers.length>0){
                                        mapView.identifyProperties.reset()
                                    }


                                    // Muhammad Ali - Create Feature and populate into popup window...
                                    ...
                                    var feature = inputFeatures2.createFeature(); // inputFeature2 will be a Feature
                                    feature.attributes.attributesJson = closestitem.attributes

                                    var popUp = ArcGISRuntimeEnvironment.createObject("Popup", {initGeoElement: feature, initPopupDefinition: popupDefinitionInfo}),
                                    popupManager = ArcGISRuntimeEnvironment.createObject("PopupManager", {popup: popUp})
                                    popupManager.objectName = "You just " + distanceArray[0].distance + " away" //identifyLayerResult.layerContent.name
                                    mapView.identifyProperties.popupManagers.push(popupManager)

                                    mapView.identifyProperties.popupDefinitions.push(popupDefinitionInfo)
                                    mapView.identifyProperties.fields.push(popupDefinitionInfo.fields)

                                    mapView.identifyProperties.features.push(feature)

                                    mapView.populateRelatedRecords(feature)
            ...
            onMouseClicked: {
                if (mapView.map.loadStatus === Enums.LoadStatusLoaded) {
                    if (measureToolIcon.checked && measurePanel.state === "MEASURE_MODE") {
                        ...
                        console.log("Mouse X and Y" + mouse.x + " : " + mouse.y)
                    }
                }
            }
            ...
            function procLayers (layers, callback) {
                ...
                for (var i=count; i--;) {
                    try {
                        ...
                        // Muhammad Ali - Get Point Layer featureset...
                        if(layer.featureCollection.json.layers[0].featureSet.geometryType==="esriGeometryPoint"){
                          layerFeatureSet = layer.featureCollection.json.layers[0].featureSet.features
                          popupDefinitionInfo =  layer.layers[0].popupDefinition
                        }
                    } catch (err) {
                        layer = layers[i]
                    }
    ...
    // Muhammad Ali location Manager Setup...
